...
- [ ] Remove exceptions from audio thread path (`processBlock` must never throw).
  - Current issue: `src/xen_processor.cpp:55`
- [ ] Replace unsafe cross-thread state handoff for non-trivial types.
...
- [ ] Split state into explicit layers:
...
- [ ] Remove UI/JUCE concerns from core state header.
  - Current coupling: `include/xen/state.hpp:12`, `include/xen/state.hpp:29`
- [ ] Remove self-include in state header.
...
- [ ] Move UI-only commands out of engine command tree.
...
- [ ] Introduce engine->UI sync boundary now:
  - `EngineSnapshot` + `commitId`
  - optional incremental patch/events
- [ ] UI consumes snapshots/events only; no direct mutation of engine internals.
...
- [ ] (pending) Initial checklist created.
