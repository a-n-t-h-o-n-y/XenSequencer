...
void append_bootstrap_specs(std::vector<CommandSpec> &specs)
{
    specs.push_back(command(
        ...
        std::make_tuple(),
        [](PluginState &, CommandExecutionContext &, CommandInvocation const &) {
            return make_result(minfo(std::string{"Welcome to XenSequencer v"} +
                                     VERSION));
        }));
    ...
    specs.push_back(command({"version"}, false,
                            ...
                            [](PluginState &, CommandExecutionContext &,
    ...
    specs.push_back(command(
        ...
        [](PluginState &ps, CommandExecutionContext &, CommandInvocation const &) {
            ...
            ps.sessions = TransformSessionState{};
    ...
    specs.push_back(history_navigation_command(
        {"undo"}, "Revert state to before the last action.",
        history_navigation_policy, HistoryNavigationDirection::Undo));
    ...
    specs.push_back(command(
        {"copy"}, false, "Copy the current selection.", copy_policy,
        std::make_tuple(), [](PluginState &ps, SubmissionEffects &effects,
            ...
            effects.write_text(
                ...
                serialize_copy_buffer_content(action::copy(
                    state, require_selection(context))));
            return unchanged_selection_result(minfo("Copied Selection"), context);
        }));

    specs.push_back(command(
        {"cut"}, false, "Cut the current selection.", cut_policy, std::make_tuple(),
        [](PluginState &ps, SubmissionEffects &effects,
           CommandExecutionContext &context, CommandInvocation const &) {
            ...
            effects.write_text(
                ...
                serialize_copy_buffer_content(action::copy(
                    state, require_selection(context))));
            auto const mutation =
                action::delete_cell(state, require_selection(context));
    ...
    specs.push_back(command(
        ...
        [](PluginState &ps, SubmissionEffects &effects,
           CommandExecutionContext &context, CommandInvocation const &) {
            ...
            auto const mutation =
                action::paste(state, require_selection(context), content);
            ps.timeline.stage(std::move(state));
    ...
    specs.push_back(command({"duplicate"}, false, "Duplicate the current selection.",
                            ...
                            [](PluginState &ps, CommandExecutionContext &context,
                               CommandInvocation const &) {
                                auto state = ps.timeline.get_state();
                                auto const mutation =
                                    action::duplicate(state, require_selection(context));
                                ps.timeline.stage(std::move(state));
                                return make_result(minfo("Selection Duplicated"),
                                                   mutation.selection);
                            }));
        ...
        command({"load", "measure"}, false,
                ...
                [](PluginState &ps, SubmissionEffects &effects,
                   ...
                   std::string const &filename) {
                    ...
                    {
                        return make_result(
                            merror("Invalid Current Sequence Directory"));
                    }
    ...
    specs.push_back(command(
        ...
        load_project_resource_policy,
        std::make_tuple(required_arg<std::string>("String", "filename")),
        [](PluginState &ps, SubmissionEffects &, CommandExecutionContext &,
    ...
    specs.push_back(command(
        ...
        [](PluginState &ps, SubmissionEffects &, CommandExecutionContext &,
           CommandInvocation const &) {
            ...
            return make_result(
                minfo("Scales Loaded: " + std::to_string(ps.library.scales.size())));
        }));
    ...
    specs.push_back(command(
        ...
        [](PluginState &ps, SubmissionEffects &, CommandExecutionContext &,
           CommandInvocation const &) {
            ...
            return make_result(
                minfo("Chords Loaded: " + std::to_string(ps.library.chords.size())));
        }));
    ...
    specs.push_back(command(
        ...
        save_measure_policy,
        std::make_tuple(required_arg<std::string>("String", "filename")),
        [](PluginState &ps, SubmissionEffects &effects, CommandExecutionContext &,
           CommandInvocation const &, std::string const &filename) {
            auto const cd = ps.config.current_sequence_directory;
            if (!cd.isDirectory())
            {
            ...
            return make_result(minfo("Measure Saved to " +
                                     single_quote(filepath.getFullPathName()
                                                      .toStdString())));
        }));
    ...
    specs.push_back(command(
        ...
        [](PluginState &, CommandExecutionContext &, CommandInvocation const &) {
            return make_result(
                minfo(get_user_library_directory().getFullPathName().toStdString()));
        }));
}
