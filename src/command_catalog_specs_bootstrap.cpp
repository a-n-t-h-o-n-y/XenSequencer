...
{
    specs.push_back(
        make_spec({"welcome"}, false, "Display welcome message.", std::make_tuple(),
    ...
    specs.push_back(make_spec(
        ...
        std::make_tuple(), [](CommandInvocation const &) { return VersionAction{}; }));
    ...
    specs.push_back(make_spec(
        ...
        [](CommandInvocation const &) { return ResetAction{}; }));

    specs.push_back(make_spec(
        {"undo"}, false, "Revert state to before the last action.", std::make_tuple(),
        [](CommandInvocation const &) { return UndoAction{}; }));

    specs.push_back(make_spec({"redo"}, false, "Reapply the last undone action.",
                              std::make_tuple(),
                              [](CommandInvocation const &) { return RedoAction{}; }));

    specs.push_back(
        make_spec({"copy"}, false, "Copy the current selection.", std::make_tuple(),
                  [](CommandInvocation const &) { return CopySelectionAction{}; }));

    specs.push_back(
        make_spec({"cut"}, false, "Cut the current selection.", std::make_tuple(),
    ...
    specs.push_back(
        make_spec({"load", "keys"}, false, "Deprecated command.", std::make_tuple(),
                  [](CommandInvocation const &) { return LoadKeysAction{}; }));

    specs.push_back(make_spec(
        {"load", "scales"}, false, "Load scales from library files.", std::make_tuple(),
        [](CommandInvocation const &) { return LoadScalesAction{}; }));

    specs.push_back(make_spec(
        {"load", "chords"}, false, "Load chords from library files.", std::make_tuple(),
        [](CommandInvocation const &) { return LoadChordsAction{}; }));

    specs.push_back(
        make_spec({"save", "measure"}, false, "Save the current measure to file.",
                  std::make_tuple(required_arg<std::string>("String", "filename")),
                  [](CommandInvocation const &, std::string filename) {
    ...
    specs.push_back(
        make_spec({"libraryDirectory"}, false,
