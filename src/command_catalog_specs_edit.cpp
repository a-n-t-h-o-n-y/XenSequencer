...
void append_edit_specs(std::vector<CommandSpec> &specs)
{
    specs.push_back(make_spec(
        {"move", "left"}, false, "Move selection left.",
    ...
    specs.push_back(make_spec(
        {"move", "right"}, false, "Move selection right.",
        std::make_tuple(optional_arg<std::size_t>("Unsigned", "amount", 1)),
        [](CommandInvocation const &, std::size_t amount) {
    ...
    specs.push_back(make_spec(
        {"move", "up"}, false, "Move selection up one level.",
        std::make_tuple(optional_arg<std::size_t>("Unsigned", "amount", 1)),
        [](CommandInvocation const &, std::size_t amount) {
    ...
    specs.push_back(make_spec(
        {"move", "down"}, false, "Move selection down one level.",
        std::make_tuple(optional_arg<std::size_t>("Unsigned", "amount", 1)),
        [](CommandInvocation const &, std::size_t amount) {
    ...
    specs.push_back(make_spec(
        {"split"}, false, "Split the current selection.",
