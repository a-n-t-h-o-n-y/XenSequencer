        ...
        command({"note"}, false, "Create a note at the current selection.",
                ...
                [](PluginState &ps, CommandExecutionContext &context,
                   ...
                   float gate) {
                    ...
                    return unchanged_selection_result(minfo("Note Created"), context);
                }));
    ...
    specs.push_back(command({"delete"}, false, "Delete the current selection.",
                            ...
                            [](PluginState &ps, CommandExecutionContext &context,
                               CommandInvocation const &) {
                                ...
                                ps.timeline.stage(std::move(state));
                                return make_result(minfo("Deleted Selection"),
                                                   mutation.selection);
                            }));
    ...
    specs.push_back(command({"lift"}, false, "Lift the current selection up one level.",
                            ...
                            [](PluginState &ps, CommandExecutionContext &context,
                               CommandInvocation const &) {
                                auto state = ps.timeline.get_state();
                                auto const mutation =
                                    action::lift(state, require_selection(context));
                                ps.timeline.stage(std::move(state));
                                return make_result(minfo("Selection Lifted One Layer"),
                                                   mutation.selection);
                            }));
}
