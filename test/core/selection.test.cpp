...
auto make_nested_measure() -> sequence::Cell
{
    auto measure = sequence::Cell{
        .elements =
            {
          ...
          "[core][selection]")
{
    ...
    {
        .path = {
            element_step(2), cell_step(3),    element_step(1),
            cell_step(1),    element_step(0), cell_step(0),
        };
    ...
    TEST_CASE("Invalid typed selection paths fail clearly", "[core][selection]")
    {
        ...
        CHECK_THROWS_AS(
            get_selected_cell_const(measure, SelectedState{.path = {cell_step(0)}}),
            std::invalid_argument);
        CHECK_THROWS_AS(
            get_selected_cell_const(
                measure, SelectedState{.path = {element_step(2), element_step(0)}}),
            std::invalid_argument);
        CHECK_THROWS_AS(
            get_selected_cell_const(
                measure, SelectedState{.path = {element_step(0), cell_step(0)}}),
            std::invalid_argument);
    }
