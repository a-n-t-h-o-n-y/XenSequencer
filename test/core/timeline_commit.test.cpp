          ...
          "[core][timeline][commit]")
{
    ...
    CHECK(timeline.redo());
    CHECK(timeline.get_state().composition.columns.at(0).pitch.transposition == 2);
}
          ...
          "[core][timeline][preview]")
{
    ...
    auto const revision = timeline.get_project_revision();
    auto state = timeline.get_state();
    state.composition.columns.at(0).pitch.transposition = 7;
